...
function GameSection({ title, imagePrefix }: { title: string, imagePrefix: string }) {
  ...
  return (
    <div className="bg-black/20 rounded-[20px] p-4 shadow-lg overflow-hidden">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-[#B09DFF] text-xs hover:text-white transition-colors cursor-pointer">
            ...
            <div key={slideIndex} className="flex gap-2 min-w-full">
              {Array.from({ length: gamesPerSlide }).map((_, gameIndex) => {
                ...
                return (
                  <div
                    key={gameIndex}
                    className="flex-1 aspect-[128/211] rounded-lg overflow-hidden group cursor-pointer"
                  >
                    <Image
